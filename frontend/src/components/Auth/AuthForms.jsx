...
function AuthForms() {
  ...
  const handleLogin = async (e) => {
    ...
    setLoginError('');
    try {
      await login(loginCedula, loginPassword);
    } catch (err) {
      ...
      <div className={`auth-container${rightPanelActive ? ' right-panel-active' : ''}`}>
        ...
        <div className="auth-form-container sign-in">
          <form className="auth-form" onSubmit={handleLogin}>
            ...
            {/* Mobile toggle link (hidden on desktop via CSS) */}
            <p className="mobile-toggle" onClick={() => setRightPanelActive(true)}>
              ¿Eres nuevo? <span>Regístrate aquí</span>
            </p>

            <div className="auth-footer">
              <Lock size={14} color="#006d37" />
              <span>Conexión cifrada AES-256</span>
            </div>
          </form>
        </div>
        ...
        <div className="auth-form-container sign-up">
          <form className="auth-form" onSubmit={handleRegister}>
            ...
            {/* Mobile toggle link (hidden on desktop via CSS) */}
            <p className="mobile-toggle" onClick={() => setRightPanelActive(false)}>
